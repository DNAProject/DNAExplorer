    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                logger.debug("{} can not get machine info now....", Thread.currentThread().getName());
            } catch(java.net.ConnectException e){
                logger.debug("{} can not get machine info now....", Thread.currentThread().getName());
            } catch (Exception e) {
                e.printStackTrace();
                logger.debug("{} get machine info error....", Thread.currentThread().getName());
            } finally {
                // 关闭链接
                if (connection != null) {
                    logger.debug("{} close http", Thread.currentThread().getName());
